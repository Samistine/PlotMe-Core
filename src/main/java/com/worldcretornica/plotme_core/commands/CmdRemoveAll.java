...
public class CmdRemoveAll extends PlotCommand {

    public CmdRemoveAll(PlotMe_Core instance) {
        super(instance);
    }
            ...
            if (manager.isPlotWorld(world)) {
                ...
                if (id.isEmpty()) {
                    ...
                    if (args.length < 2 || !args[1].equals("confirm")) {
                        player.sendMessage(C("WordUsage") + ": §c/plotme removeall confirm");
                    } else {
                        ...
                        if (plot.getOwnerId().equals(playerUniqueId) || player.hasPermission(PermissionNames.ADMIN_REMOVE)) {
                            if (plot.allowedcount() > 0) {
                                    ...
                                    if (!event.isCancelled()) {
                                        ...
                                        if (isAdvancedLogging()) {
                                            serverBridge.getLogger()
                                                    .info(allowed + " " + C("MsgRemovedPlayer") + " " + allowed + " " + C("MsgFromPlot") + " " + id + (price != 0 ? " " + C("WordFor") + " " + price : ""));
                                        }
                                    }
                                ...
                                player.sendMessage("No players are added to this plot"); //TODO: Localization
                            }
                        } else {
